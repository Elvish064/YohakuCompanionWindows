...
“显示别名”只改变对外公开名称，不会改变应用识别标识。敏感词规则在别名处理后执行，因此也能过滤别名。
...
数据库只保存非秘密连接元数据、来源设置、应用规则、敏感词规则、暂停状态和设备序列。客户端不会读取、迁移或删除 macOS Yohaku Companion 或 ProcessReporter 的数据。
...
先直接请求完整能力 URL。如果 `curl.exe` 返回 200 而客户端返回 403，请确认运行的是最新客户端；客户端会发送 `YohakuCompanion/1.7.9 (Windows)` User-Agent，并对局域网 HTTP 绕过环境代理。还应检查反向代理、WAF 或爬虫防护是否按 User-Agent、来源或路径拦截。
